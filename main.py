...
def delete_original_images(folder_path):
    ...
    # 询问是否要备份要删除的文件
    if input(f"\n{Fore.YELLOW}是否要备份这些即将删除的文件？(y/n，默认y): {Style.RESET_ALL}").strip().lower() != 'n':
        if not backup_files_to_delete(folder_path, files_to_delete):
            if input(f"\n{Fore.RED}备份失败，是否继续删除操作？(y/n，默认n): {Style.RESET_ALL}").strip().lower() != 'y':
                return 0
...
def show_warning():
    ...
    print(f"{Fore.RED}1. 此脚本的操作会修改、替换、删除您的文件，请确保数据已备份{Style.RESET_ALL}")
    ...
    backup_response = input(f"\n{Fore.YELLOW}是否需要创建备份？(y/n，默认y): {Style.RESET_ALL}").strip().lower()
...
def replace_image_references(markdown_path, need_confirm=True):
    ...
    try:
                    ...
                    if 'image' in yaml_data:
                        ...
                        if isinstance(img_path, str):
                            ...
                            if ext in ['.png', '.jpg', '.jpeg', '.bmp', '.tiff']:
                                ...
                                if os.path.exists(os.path.join(os.path.dirname(markdown_path), webp_path)):
                                    print(f"\n处理文件: {markdown_path}")
        ...
        # 匹配各种图片引用格式
        img_patterns = [
            (r'!\[([^\]]*?)\]\(([^)]+?)\)', 'Markdown格式'),  # Markdown 标准格式 ![alt](path)
            (r'<img.*?src=[\'\"](.*?)[\'\"]', 'HTML格式'),  # HTML 格式 <img src="path">
            (r'(?:\"|\s)([^"\s]+\.(?:jpg|jpeg|png|bmp|tiff))(?:\"|\s|$)', '直接引用'),  # 裸露的图片路径，包括引号包围的情况
            (r'{{<\s*(?:imgrow|music)\s+(?:[^>}]*?\s+)?(?:img=)?\"([^"]+\.(?:jpg|jpeg|png|bmp|tiff))\"', 'Hugo Shortcode'),  # Hugo shortcode中的img属性
        ...
        def process_image_path(img_path, md_dir):
            ...
            # 检查webp文件是否存在
            full_webp_path = os.path.join(md_dir, webp_path) if not os.path.isabs(webp_path) else webp_path
        ...
        def replace_match(pattern_info, content, md_dir):
            ...
            def collect_replacement(match):
                if pattern_type == 'Hugo Imgrow':
                    ...
                    img_path = match.group(2) if pattern_type == 'Markdown格式' else match.group(1)
                    webp_path = process_image_path(img_path, md_dir)
                    if webp_path:
                    ...
                    webp_path = process_image_path(img_path, md_dir)
                    if webp_path:
                        before, after = get_context(content, match.start(1), match.end(1))
                        replacements.append((
                            img_path,
                            webp_path,
                            img_path,
            ...
            if replacements:
                # 检查是否有直接引用的情况
                has_direct_refs = any(r[5] in ['直接引用', 'Hugo Imgrow内图片'] for r in replacements) # Imgrow 是我主题的shortcode 用来展示图片的
                ...
                if not need_confirm and has_direct_refs:
                    ...
                    print(f"{Fore.YELLOW}例如：如果文中有 555.jpg 这样的假装有表情包的表达，但文件夹中没有 555.webp ，则不会被替换。{Style.RESET_ALL}")
                    ...
                    for old, new, old_path, new_path, context, ref_type in replacements:
                        if ref_type in ['直接引用', 'Hugo Imgrow内图片']:
                            ...
                            print(f"\n类型：{ref_type}")
                            print(f"上下文示例：...{before}{Fore.RED}{old_path}{Style.RESET_ALL}{after}...")
                            print(f"将替换为：...{before}{Fore.GREEN}{new_path}{Style.RESET_ALL}{after}...")
                            print(f"{Fore.GREEN}（已确认存在对应的webp文件）{Style.RESET_ALL}")
                            example_shown = True
                            break
                    ...
                    if example_shown:
                        ...
                        if response != 'y':
                            # 移除所有直接引用的替换
                            replacements = [r for r in replacements if r[5] not in ['直接引用', 'Hugo Imgrow内图片']]
                        else:
                            # 保险
                            confirm_response = input("是否要逐个确认这些替换？(y/n): ").strip().lower()
                ...
                if replacements:
                    ...
                    for old, new, old_path, new_path, context, ref_type in replacements:
                        ...
                        if ref_type in ['直接引用', 'Hugo Imgrow内图片']:
                            before, after = context
                            print(f"上下文：...{before}{Fore.RED}{old_path}{Style.RESET_ALL}{after}...")
                            print(f"替换为：...{before}{Fore.GREEN}{new_path}{Style.RESET_ALL}{after}...")
                            print(f"{Fore.GREEN}（已确认存在对应的webp文件）{Style.RESET_ALL}")
                        else:
        ...
        for pattern_info in img_patterns:
            new_main_content, changed = replace_match(pattern_info, main_content, md_dir)
            if changed:
...
def convert_to_webp(image_path, lossless=False, need_confirm=True, quality=80):
    ...
    try:
        ...
        if os.path.exists(output_path):
            if need_confirm:
                response = input(f"\n文件 {output_path} 已存在，是否覆盖？(y/n/gg，默认n): ").strip().lower()
                if response == 'gg':
                    return 'gg'
                if response != 'y':
                    print("跳过该文件")
...
def process_images(folder_path):
    ...
    if not use_lossless:
        print("\n将使用有损压缩模式")
        quality_input = input(f"请输入压缩质量(1-100，默认{quality}，直接回车使用默认值): ").strip()
        if quality_input:
            try:
                quality = int(quality_input)
                if quality < 1 or quality > 100:
                    print(f"{Fore.YELLOW}输入的质量值无效，将使用默认值 80{Style.RESET_ALL}")
                    quality = 80
            except ValueError:
                print(f"{Fore.YELLOW}输入的质量值无效，将使用默认值 80{Style.RESET_ALL}")
    ...
    # 保险
    need_confirm = input(f"\n{Fore.YELLOW}是否需要逐个确认覆盖已存在的文件？(y/n): {Style.RESET_ALL}").strip().lower() == 'y'
    if not need_confirm:
        print("\n将自动覆盖所有已存在的文件")
    else:
    ...
    for image_path in image_files:
        result = convert_to_webp(image_path, lossless=use_lossless, need_confirm=need_confirm, quality=quality)
        if result == 'gg':
            print(f"\n{Fore.YELLOW}切换到自动覆盖模式{Style.RESET_ALL}")
            need_confirm = False
            # 使用自动模式
            if convert_to_webp(image_path, lossless=use_lossless, need_confirm=False, quality=quality):
                converted_count += 1
...
def main():
    ...
    while True:
        ...
        if choice == '4':
            ...
            print("1. 脚本会在指定文件夹中搜索所有图片文件（jpg、png、jpeg、bmp、tiff）")
            print("2. 检查每个图片是否存在同名的 webp 文件")
            print("   例如：对于 image.jpg，检查是否存在 image.webp")
            print("3. 如果找到同名的 webp 文件，则将原始图片（如 image.jpg）加入删除列表")
            print("4. 删除操作不可逆，建议先进行备份")
            
            if input(f"\n{Fore.RED}您确定要继续删除操作吗？(y/n): {Style.RESET_ALL}").strip().lower() == 'y':
